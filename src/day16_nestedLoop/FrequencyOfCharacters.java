    ...
    public static void main(String[] args) {
        ...
        for (int j = 0; j < str.length(); j++) {
            ...
            for (int i = 0; i < str.length(); i++) {                         //to find the frequency of each character
                char each = str.charAt(i);                                   //each character of str (outer loop)is
                if(ch == each) {count++;}                                    //compared with inner loop's each char.
            }
            if(result.contains(""+ch)) { continue;}                         //inner loop'un ilk seferinden sonra
            result += ch;
            result += count;
        }
